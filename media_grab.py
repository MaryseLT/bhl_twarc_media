...
def fetch_media_for_feed(feed_dict):
                   ...
                   status_code = urllib.request.urlopen(url)
                    ...
                    if e.getcode() !=200:
                        if e.getcode() ==403:
                            ...
                            logger.info("ERROR w/ Tweet Media > {0} {1}".format(execption,url))
                            dead_tweets.append(url)
                ...
                try:
                    ...
                    if e.getcode() !=200:
                        if e.getcode() ==403:
                            ...
                            logger.info("ERROR w/ Profile Media > {0} {1}".format(e,url))
                            dead_profiles.append(url)
                            #   Account for ConnectError

            # Check if ID folder exists
                if not os.path.exists(profile_dir):
                    logger.info("CREATNG profile_dir > {0}".format(profile_dir))
                    os.makedirs(profile_dir)

            # Check if image exists in ID folder
                if filename not in os.listdir(profile_dir):
                    logger.info("FETCHING Profile Media > {0}: {1}".format(profile_dir,url))
    ...
    logger.info("Finished Downloads > ", end)
